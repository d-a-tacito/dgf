...
def rosenbrock_discrete(
    ...
    Логика алгоритма:
      - Имеем начальную точку x0, значение функции f(x0) и начальный шаг.
      - Для каждого шага проверяем все направления (для 2D – 4 направления, для nD – 2n направлений).
      - Если по какому-либо направлению (x_new = x + step*d) значение f уменьшается,
        то переходим в эту точку и повторяем поиск с этой новой точки.
      - Если во всех направлениях функция не уменьшается, шаг уменьшается.
    ...
    # Формируем список направлений: вдоль каждой оси – в положительную и отрицательную стороны.
    directions = []
    for i in range(n):
        e = np.zeros(n)
        e[i] = 1
    ...
    while step > tol and iter_num < max_iter:
        ...
        for d in directions:
            ...
            # Если улучшение найдено – переходим к новой точке
            if f_new < f_current:
                x_current = x_new
                f_current = f_new
                trajectory.append(x_current.copy())
                improved = True
                break  # выходим из цикла направлений и начинаем с новой точки
        # Если улучшения по всем направлениям нет – уменьшаем шаг
        if not improved:
